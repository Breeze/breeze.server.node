...
export class SequelizeSaveHandler {
  ...
  save(): Promise<{ errors: ServerEntityError[], message: string }> | Promise<Entity[]> {
    ...
    let entityInfos = this.entitiesFromClient.map(entity => {
      ...
      let entityTypeName = (entityAspect as any).entityTypeName;
  ...
  private _saveWithTransaction(saveMap: SaveMap) {
    let sequelize = this.sequelizeManager.sequelize;
    return sequelize.transaction().then(trx => {
      // this.transaction = trx;

      let beforeSaveEntities = (this.beforeSaveEntities || noopBeforeSaveEntities).bind(this) as (sm: SaveMap, trx: Transaction) => SaveMap;
  ...
  private _saveCore(saveMap: SaveMap, transaction: Transaction): Promise<{ errors: ServerEntityError[], message: string }> | Promise<Entity[]> {
    ...
    let entityGroups = sortedEntityTypes.map((entityType: EntityType) => {
      return { entityType: entityType, entityInfos: saveMap[entityType.name] } as EntityGroup;
    });
  ...
  private _processEntityGroup(entityGroup: EntityGroup, transaction: Transaction, processDeleted: boolean): Promise<Entity[]> {
  ...
  private _saveEntityAsync(entityInfo: EntityInfo, sqModel: { new(): Model } & typeof Model, transaction: Transaction): Promise<Entity> {
    // function returns a promise for this entity
    // and updates the results array.
  ...
  private _addToResults(entity: Entity, entityTypeName: string) {
    (entity as any).$type = entityTypeName;
    this._savedEntities.push(entity);
    return entity;
  }

  private _coerceData(entity: Entity, entityType: EntityType) {
...
function noopBeforeSaveEntity(entityInfo: EntityInfo) {
  return true;
}
...
function toposortEntityInfos(entityType: EntityType, entityInfos: EntityInfo[]) {
  let edges: [EntityInfo, EntityInfo][] = [];
...
function handleItemSaveError(entity: Entity, entityState: EntityState | string) {
